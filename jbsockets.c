...
static jb_socket rfc2553_connect_to(const char *host, int portnum, struct client_state *csp)
{
   ...
   if ((-1 == retval) || (sizeof(service) <= retval))
   {
      log_error(LOG_LEVEL_ERROR,
         ...
         portnum);
      csp->error_message = _strdup("Invalid port number");
      csp->http->host_ip_addr_str = _strdup("unknown");
   ...
   if ((retval = getaddrinfo(host, service, &hints, &result)))
   {
      ...
      errno = EINVAL;
      csp->error_message = _strdup(gai_strerror(retval));
      csp->http->host_ip_addr_str = _strdup("unknown");
      return(JB_INVALID_SOCKET);
   }
   ...
   if (!rp)
   {
      ...
      csp->error_message = _strdup(strerror(socket_error));
      return(JB_INVALID_SOCKET);
   }
...
static jb_socket no_rfc2553_connect_to(const char *host, int portnum, struct client_state *csp)
{
   ...
   csp->http->host_ip_addr_str = _strdup(inet_ntoa(inaddr.sin_addr));
...
#else
      *ip_address = _strdup(inet_ntoa(server.sin_addr));
...
#undef client
#else
   csp->ip_addr_str  = _strdup(inet_ntoa(client.sin_addr));
