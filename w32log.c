   ...
   if (g_bHighlightMessages)
   {
      ...
      for (i = 0; patterns_to_highlight[i].str != NULL; i++)
      {
         if (regexec(&patterns_to_highlight[i].buffer, pszText, 1, &match, 0) == 0)
         {
            ...
            {
               pszAfter = _strdup(&pszText[match.rm_eo]);
            }
...
HWND CreateLogWindow(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hwnd = CreateWindowExA(WS_EX_APPWINDOW, szWndName, szWndTitle,
      WS_OVERLAPPEDWINDOW, 100, 100, 600/*CW_USEDEFAULT*/,
      /*CW_USEDEFAULT*/300, hwndOwner, NULL, hInstance, NULL);
