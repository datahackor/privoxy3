                                                ...
                                                const char * path)
{
   ...
   if (NULL == (path_copy = _strdup(path)))
   {
                                     ...
                                     const char *templatename)
{
   ...
   {
      rsp->status = _strdup("404 No such domain");
   ...
   else if (!strcmp(templatename, "forwarding-failed"))
   {
      ...
      if (NULL == csp->error_message)
      {
         ...
         csp->error_message = _strdup("Failure reason missing. Check the log file for details.");
      }
...
jb_err cgi_redirect (struct http_response * rsp, const char *target)
{
   ...
   rsp->status = _strdup("302 Local Redirect from Privoxy");
   if (rsp->status == NULL)
   {
      return JB_ERR_MEMORY;
   }
                     ...
                     const char *templatename, int recursive)
{
   ...
   *template_ptr = NULL;
   ...
   {
      templates_dir_path = _strdup(csp->config->templdir);
   }
   else
   {
...
struct map *default_exports(const struct client_state *csp, const char *caller)
{
   ...
   if (csp->config->hostname)
   {
      ...
      hostname = _strdup(csp->config->hostname);
   }
   else
   {
